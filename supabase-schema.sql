...
create policy "Lists visible" on public.lists for select using (is_public or auth.uid() = user_id);
...
create policy "List items visible" on public.list_items for select
  using (auth.uid() = user_id or exists (select 1 from public.lists l where l.id = list_items.list_id and l.is_public));
